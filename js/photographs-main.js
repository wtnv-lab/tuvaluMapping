...
var photoBillboards;
var photoBillboardMeta = {};
var photoBillboardUid = 0;
...
function addPhotoBillboardsFromCzml(czmlPackets) {
	for (var p = 0; p < czmlPackets.length; p++) {
		var packet = czmlPackets[p];
		if (!packet.billboard || !packet.position || !packet.position.cartographicDegrees) {
			continue;
		}

		var cartographicDegrees = packet.position.cartographicDegrees;
		if (!cartographicDegrees || cartographicDegrees.length < 2) {
			continue;
		}

		var lon = Number(cartographicDegrees[0]);
		var lat = Number(cartographicDegrees[1]);
		var height = Number(cartographicDegrees[2] || 0);
		if (!isFinite(lon) || !isFinite(lat)) {
			continue;
		}

		var imageUrl = getCzmlImage(packet.billboard.image);
		if (!imageUrl) {
			continue;
		}

		var key = 'photo-' + (photoBillboardUid++);
		photoBillboardMeta[key] = {
			name: getCzmlString(packet.name),
			description: getCzmlString(packet.description)
		};

		photoBillboards.add({
			id: key,
			position: Cesium.Cartesian3.fromDegrees(lon, lat, height + 2.0),
			image: imageUrl,
			scale: 1.0,
			verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
			disableDepthTestDistance: Number.POSITIVE_INFINITY,
			scaleByDistance: new Cesium.NearFarScalar(1.5e3, 1.2, 2.0e5, 0.65)
		});
	}
}
...
function buildPickedInfoHtml(element) {
	var primitiveId = '';
	var primitiveMeta = null;
	if (element.primitive && element.primitive.id) {
		primitiveId = String(element.primitive.id);
		primitiveMeta = photoBillboardMeta[primitiveId] || null;
	}
	var name = primitiveMeta ? primitiveMeta.name : '';
	var safeName = $('<div>').text(name).html();
	var descriptionHtml = primitiveMeta ? (primitiveMeta.description || '') : '';
...
function loadCesium(){
	...
	photoBillboards = scene.primitives.add(new Cesium.BillboardCollection());
...
function loadCzml(){
	...
	var load = setInterval(function(){
		$.getJSON(czmlDataArray[i].url, function (czmlPackets) {
			addPhotoBillboardsFromCzml(czmlPackets);
		}).fail(function () {
